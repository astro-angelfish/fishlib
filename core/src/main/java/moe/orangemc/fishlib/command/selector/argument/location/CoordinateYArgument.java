...
public class CoordinateYArgument implements SelectorArgument<Double> {
	@Override
	...
	public boolean matchEntity(Selector selector, Entity entity, Double value, Boolean parallelResult) {
		...
		locationContext.setY(value);

		if (Math.round(value) == value) {
			return entity.getLocation().getBlockY() == (int) Math.round(value);
		}
		return Math.abs(entity.getLocation().getY() - value) < 0.001;
	}
